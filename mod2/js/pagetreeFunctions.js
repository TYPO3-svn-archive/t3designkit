...
function cloneItem(el) {
    window.clearTimeout(window.t3dkptwTimer);
    top.t3dkptwel 	= el;
    top.t3dkptwclone 	= true;
    addItem2();
}
					    
function addItem(el) {
    top.t3dkptwel 	= el;
    window.t3dkptwTimer 	= window.setTimeout('addItem2()',500);
}
...
function addItem2() {
    ...
    nel.lastChild.id = nel.lastChild.id.replace(/t3dkptw[0-9]*/,nId);
    nel.lastChild.previousSibling.id = nel.lastChild.previousSibling.id.replace(/t3dkptw[0-9]*/,nId);
    nel.lastChild.previousSibling.previousSibling.id = nel.lastChild.previousSibling.previousSibling.id.replace(/t3dkptw[0-9]*/,nId);

    gpel.insertBefore(nel,pel.nextSibling);    

    if(top.t3dkptwclone==false) {
	...
	for(i=0;i<pel.nextSibling.getElementsByTagName("li").length;i++) {	
	    ...
	    ens = en.substr(en.search(/\[[0-9]*\]\[title\]/),en.length);
    	    ...
    	    pel.nextSibling.getElementsByTagName("li")[i].id = pel.nextSibling.getElementsByTagName("li")[i].id.replace(/t3dkptw[0-9]*/,nId+i);
    	    pel.nextSibling.getElementsByTagName("li")[i].lastChild.id = pel.nextSibling.getElementsByTagName("li")[i].lastChild.id.replace(/t3dkptw[0-9]*/,nId+i);
    	    pel.nextSibling.getElementsByTagName("li")[i].lastChild.previousSibling.id = pel.nextSibling.getElementsByTagName("li")[i].lastChild.previousSibling.id.replace(/t3dkptw[0-9]*/,nId+i);
    	    pel.nextSibling.getElementsByTagName("li")[i].lastChild.previousSibling.previousSibling.id = pel.nextSibling.getElementsByTagName("li")[i].lastChild.previousSibling.previousSibling.id.replace(/t3dkptw[0-9]*/,nId+i);
	}
    }
...
function addSubItem(el,clone) {
    ...
    if(nc.id.search(/down/)!=-1 || nc.id.search(/up/)!=-1) {
        nc.id = nc.id.replace(/t3dkptw[0-9]*/,nId);
        nc.style.display = "none";
    }
    if(nc.previousSibling.id.search(/up/)!=-1) {
	nc.previousSibling.id = nc.previousSibling.id.replace(/t3dkptw[0-9]*/,nId);
